"""
Pipeline đồng nhất: Raw → Tranh sơn dầu → Resize/Crop/Chuẩn hóa màu → Flip/Rotation/Color Jitter
→ Lưu tất cả vào một folder: dataset/output_image_style/

Flow:
  raw (ảnh input)
  → oil painting transfer
  → resize, crop, chuẩn hóa màu (processed)
  → flip, rotation, color jitter (augmented)
  → output_image_style/ (một folder chứa toàn bộ ảnh tranh sơn dầu đã xử lý + augmentation)
"""
...
# Cho phép import từ scripts/ khi chạy từ thư mục gốc project
...
from oil_painting_transfer import transfer_to_oil_painting
...
def process_oil_image(
    img: np.ndarray,
    target_size: tuple[int, int],
    use_crop: bool,
    use_color_norm: bool,
) -> np.ndarray:
    """Resize, crop (tùy chọn), chuẩn hóa màu (tùy chọn) trên ảnh tranh sơn dầu."""
    if use_crop:
        try:
...
def run_pipeline(
    ...
    **oil_kw,
    ...
    """
    Chạy pipeline: raw → oil painting → resize/crop/color norm → augmentation → output_image_style.

    Args:
        input_folder: Thư mục ảnh raw đầu vào.
        output_folder: Thư mục đầu ra (mặc định: dataset/output_image_style).
        prefix: Tiền tố tên file (vd: sondau_style).
        ...
        **oil_kw: Tham số truyền vào transfer_to_oil_painting (brush_size, oil_blend, ...).
    """
    ...
    print(f"--- Pipeline: {in_dir} → {out_dir} ---")
    ...
    for path in files:
        ...
        # 1. Oil painting transfer
        oil_img = transfer_to_oil_painting(img, **oil_kw)

        # 2. Processed: resize, crop, color norm
        processed = process_oil_image(
            oil_img,
...
def main():
    parser = argparse.ArgumentParser(
        description="Pipeline: Raw → Oil painting → Resize/Crop/Color norm → Flip/Rotation/Jitter → output_image_style"
    )
    parser.add_argument(
        "input",
        type=str,
